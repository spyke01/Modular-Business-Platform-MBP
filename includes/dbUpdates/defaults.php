...
if ( count( $defaultMenuItems ) ) {
	foreach ( $defaultMenuItems as $name => $infoArray ) {
		$result = $ftsdb->select( DBTABLEPREFIX . "menus",
			"name = :name",
			[
				":name" => $name,
			] );
...
if ( count( $defaultMenuItems ) ) {
	foreach ( $defaultMenuItems as $link => $infoArray ) {
		$result = $ftsdb->select( DBTABLEPREFIX . "menu_items",
			"link = :link AND menu_id = :menu_id ORDER BY id",
			[
				":link"    => $link,
				":menu_id" => $infoArray['menu_id'],
			] );
		if ( ! $result || count( $result ) == 0 ) {
			$result2 = $ftsdb->insert( DBTABLEPREFIX . 'menu_items',
				[
					"text"      => $infoArray['text'],
					"menu_id"   => $infoArray['menu_id'],
		...
		if ( count( $result ) > 1 ) {
			...
			$result2 = $ftsdb->delete( DBTABLEPREFIX . 'menu_items',
				"link = :link AND menu_id = :menu_id AND id > :firstID",
				[
					":link"    => $link,
					":menu_id" => $infoArray['menu_id'],
					":firstID" => $result[0]['id'],
...
$result = $ftsdb->update( DBTABLEPREFIX . "menus",
	[
		"added_by" => 'System',
	],
