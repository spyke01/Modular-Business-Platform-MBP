...
function perform_login($username, $encryptedPassword, $userID = '') {
	...
	$result = $ftsdb->select(USERSDBTABLEPREFIX . "users", $sql, array(
		":id" => $userID,
		":username" => $username,
...
function getMyUserIDs($reset = false) {
	...
	if ( empty( $myUsers ) ) {
		...
		$result = $ftsdb->select(USERSDBTABLEPREFIX . "users", $extraSQL, array(
			":id" => $_SESSION['userid']
		));
...
function username_exists( $username ) {
	...
	$exists = 0;
	$results = $ftsdb->select(DBTABLEPREFIX . "users", "username = :username", array(
		":username" => $username,
	));
...
function getUserCount() {
	...
	$totalUsers = 0;
	$results = $ftsdb->select( DBTABLEPREFIX . "users", "", array(), 'COUNT(id) AS totalUsers' );
...
//=================================================
function printUsersTable($searchVars = array()) {
	...
	$result = $ftsdb->select(USERSDBTABLEPREFIX . "users", $extraSQL . " ORDER BY signup_date DESC", array(
	...
	$table = new tableClass('', '', '', "table table-striped table-bordered tablesorter", "usersTable");
	
	// Create table title
	$table->addNewRow( array( array( 'data' => "Current Users (" . count($result) . ")", "colspan" => $numOfColumns ) ), '', 'title1', 'thead');
	...
	// Add our data
	if ( !$result ) {
		$table->addNewRow( array( array( 'data' => "There are no users in the system.", "colspan" => $numOfColumns ) ), "usersTableDefaultRow", "greenRow" );
	} else {
		foreach ( $result as $row ) {	
			$rowData = array();
			...
			foreach ( $columns as $column_name => $column_display_name ) {
				switch ( $column_name ) {
					...
					case 'email_address':
						...
						break;
					case 'full_name':
						$rowData[] = array( 'data' => $row['first_name'] . ' ' . $row['last_name'] );
						break;
					case 'signup_date':
						$rowData[] = array( 'data' => $row['signup_date'] );
						break;
					case 'user_level':
...
function printNewUserForm() {
	...
	$formFields = apply_filters( 'form_fields_users_new', array(
		'first_name' => array(
			'text' => 'First Name',
			'type' => 'text',
			'class' => 'required',
	...
	return makeForm('newUser', il( $menuvar['USERS'] ), 'New User', 'Create User', $formFields, array(), 1);
}
...
function printEditUserForm($userID) {
	...
	if ( canAccessUser($userID) ) {
		$result = $ftsdb->select(USERSDBTABLEPREFIX . "users", "id = :id LIMIT 1", array(
			":id" => $userID
		));
		...
		if ($result && count($result) == 0) {
			...
			$formFields = apply_filters( 'form_fields_users_edit', array(
				'first_name' => array(
					'text' => 'First Name',
					'type' => 'text',
					'class' => 'required',
...
function printCreateAccountForm() {
	...
	$formFields = apply_filters( 'form_fields_account_create', array(
		'email_address' => array(
			'text' => 'Email Address',
	...
	return makeForm('createAccount', il( $menuvar['CREATEACCOUNT'] ), 'Create Account', 'Create Account', $formFields, array(), 1, 0, '', $extraFormOptions);
}
...
function printForgotPasswordForm() {
	...
	$formFields = apply_filters( 'form_fields_account_forgot_password', array(
		'email_address' => array(
			'text' => 'Email Address',
			'placeholder' => 'Email Address',
			'type' => 'text',
	...
	return makeForm('forgotPassword', il( $menuvar['FORGOTPASSWORD'] ), 'Forgot Password?', 'Reset Password', $formFields, array(), 1, 0, '', $extraFormOptions);
}
...
function printResetPasswordForm( $token ) {
	...
	$formFields = apply_filters( 'form_fields_account_reset_password', array(
		'token' => array(
			'value' => $token,
			'type' => 'hidden',
		),
	...
	return makeForm('resetPassword', il( $menuvar['FORGOTPASSWORD'] . '&token=' . $token ), 'Reset Password', 'Update Password', $formFields, array(), 1, 0, '', $extraFormOptions);
}
...
function printSearchUsersForm($searchVars) {
	...
	$formFields = apply_filters( 'form_fields_users_search', array(
		'notice' => array(
			'value' => '<strong>Choose any or all of the following to search by.</strong>',
			'type' => 'html',
		),
		'search_first_name' => array(
			'text' => 'First Name',
			'type' => 'text',
		),
		'search_last_name' => array(
			'text' => 'Last Name',
			'type' => 'text',
		),
		'search_email_address' => array(
			'text' => 'Email Address',
			'type' => 'text',
		),
