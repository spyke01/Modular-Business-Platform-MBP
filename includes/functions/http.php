...
function il( $link, $fullLink = 0 ) {
	...
	if ( ! empty( $newLink ) && $fullLink && ! stristr( $newLink, 'http://' ) && ! stristr( $newLink, 'https://' ) ) {
		$newLink = 'http' . ( isset( $_SERVER['HTTPS'] ) ? 's' : '' ) . '://' . $_SERVER['HTTP_HOST'] . rtrim( dirname( $_SERVER['PHP_SELF'] ), '/\\' ) . $newLink;
	}
...
function createRewriteURL( $link ) {
	...
	if ( ! empty( $link ) ) {
		...
		if ( $queryArray['prefix'] ) {
			$moduleRewriteInfo = callModuleHook( $queryArray['prefix'],
				'moduleRewriteInfo',
				array(
			...
			if ( is_array( $moduleRewriteInfo ) && count( $moduleRewriteInfo ) > 0 ) {
				if ( isset( $moduleRewriteInfo['rewriteArray'] ) ) {
					$rewriteArray = $moduleRewriteInfo['rewriteArray'];
				}
				if ( isset( $moduleRewriteInfo['linkPrefix'] ) ) {
					$newLink = $moduleRewriteInfo['linkPrefix'];
				}
				if ( isset( $moduleRewriteInfo['useHTMLSuffix'] ) ) {
					$useHTMLSuffix = $moduleRewriteInfo['useHTMLSuffix'];
				}
			}
		}
		...
		if ( count( $rewriteArray ) ) {
			foreach ( $rewriteArray as $key => $value ) {
				// Check $value=
				if ( ! empty( $queryArray[ $value ] ) ) {
					$newLink .= $queryArray[ $value ] . '/';
				}
			}
		}
		...
		if ( count( $queryArray ) ) {
			...
			if ( ! empty( $extraItems ) ) {
				$newLink .= '?' . substr( $extraItems, 1 );
			}
		}
		if ( ! empty( $newLink ) && $useHTMLSuffix ) {
			$newLink = rtrim( $newLink, '/' ) . '.html';
		}
		if ( ! empty( $newLink ) ) {
			$newLink = '/' . $newLink;
		}
	...
	if ( empty( $newLink ) ) {
		$newLink = 'http' . ( isset( $_SERVER['HTTPS'] ) ? 's' : '' ) . '://' . $_SERVER['HTTP_HOST'] . rtrim( dirname( $_SERVER['PHP_SELF'] ), '/\\' ) . '/';
	}
...
function parseRewrites() {
	...
	// Get our rewrites
	$result = $ftsdb->select( DBTABLEPREFIX . "rewrites", "1 ORDER BY prefix DESC", [], '`match`, `query`' );
...
function add_url_rewrite( $match, $query, $added_by = 'System', $prefix = '' ) {
	...
	$result = $ftsdb->insert( DBTABLEPREFIX . 'rewrites',
		array(
			"match"    => $match,
			"query"    => $query,
			"added_by" => $added_by,
...
function delete_url_rewrite( $match ) {
	...
	$result = $ftsdb->delete( DBTABLEPREFIX . 'rewrites',
		"`match` = :match",
		array(
			":match" => $match,
		) );
...
function update_url_rewrite( $match, $query ) {
	...
	$result = $ftsdb->update( DBTABLEPREFIX . 'rewrites',
		array(
			":query" => $query,
		),
		"`match` = :match",
...
function url_rewrite_exists( $match ) {
	...
	$exists  = 0;
	$results = $ftsdb->select( DBTABLEPREFIX . "rewrites",
		"`match` = :match",
		array(
			":match" => $match,
		) );
...
// DEPRECATED
//=========================================================	
function returnRemoteFilePost( $host, $directory, $filename, $urlVariablesArray = [] ) {
	global $fts_http;
