...
function getNextWidgetInstanceNumber($name) {
	...
	$result = $ftsdb->select(DBTABLEPREFIX . "widgets", "widget_id LIKE :widget_id ORDER BY `id` DESC LIMIT 1", array(
		":widget_id" => $name . '%'
	), 'widget_id');
...
function listWidgetsByArea($area) {
	...
	$result = $ftsdb->select(DBTABLEPREFIX . "widgets", "area = :area ORDER BY `order`", array(
		":area" => $area
	));
	...
	if ($result) {
		foreach ($result as $row) {
			if (isset($available_widgets[$row['type']])) {
					...
					if ( $actual_page_id == 'admin' && ( isset($instance->displayInAdminSection) && $instance->displayInAdminSection == 0 ) && $page->getTemplateVar('Theme') == $mbp_config['ftsmbp_theme']) continue; 
