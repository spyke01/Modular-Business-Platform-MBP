...
function deleteUserNotification( $id ) {
	...
	$result = $ftsdb->delete( DBTABLEPREFIX . 'notifications',
		"id = :id",
		array(
			":id" => $id,
		) );
...
function pruneUserNotifications() {
	...
	$result = $ftsdb->delete( DBTABLEPREFIX . 'notifications',
		"created <= DATE_SUB( CURDATE( ) , INTERVAL :months MONTH )",
		array(
...
function printUserNotificationsTable( $userID ) {
	...
	$table = new Table( '', '', '', "table table-striped table-bordered tablesorter", "notificationsTable" );
