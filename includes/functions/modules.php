 ...
 * @param array $hookArguments (default: array())
 ...
 */
function callModuleHook($prefix, $hookToCall, $hookArguments = array(), $activeModulesOnly = 1, $returnType = 'string') {
	...
	if ($prefix == '') { $modulesToCall = array_keys( $modules ); } else { $modulesToCall = array( $prefix ); }
	
	if ($returnType == 'array') { $returnVar = array(); } else { $returnVar = ""; }
	...
	foreach($modulesToCall as $prefix) {
		...
		if (method_exists($modules[$prefix], $hookToCall)) {
			...
			if ( !empty( $hookArguments ) ) {	$hookResult = $modules[$prefix]->$hookToCall($hookArguments); } else { $hookResult = $modules[$prefix]->$hookToCall(); }
					
			if ( $returnType == 'array' ) { $returnVar = $returnVar + (array) $hookResult; } else { $returnVar .= $hookResult; }
		}
	}
...
function initializeModules() {
	...
	$result = $ftsdb->select(DBTABLEPREFIX . "modules", "", array(), 'active, prefix');
...
function installModule($prefix, $name, $description, $developer, $version) {
	...
	// Do the work
	$result = $ftsdb->select(DBTABLEPREFIX . "modules", "prefix = :prefix", array(
		":prefix" => $prefix
	));
	if ( count( $result ) == 0 ) {
		$result2 = $ftsdb->insert(DBTABLEPREFIX . 'modules', array(
			"name" => $name,
...
function uninstallModule($prefix) {
	...
	$result = $ftsdb->delete(DBTABLEPREFIX . 'modules', "prefix = :prefix", array(
		":prefix" => $prefix
	));
}
...
function activateModule($prefix) {	
	...
	$result = $ftsdb->update(DBTABLEPREFIX . "modules", array(
			"active" => ACTIVE
		), 
		"prefix = :prefix", array(
			":prefix" => $prefix
