...
//=========================================================
function theme_register_options($theme_folder, $theme_config = array()) {
...
function printThemesTable() {
	...
	$table = new tableClass('', '', '', "table table-striped table-bordered tablesorter", "themesTable");
	
	// Create table title
	$table->addNewRow(array(array('data' => "Available Themes", "colspan" => "5")), '', 'title1', 'thead');
	
	// Create column headers
	$table->addNewRow(
		array(
			array('type' => 'th', 'data' => "Preview"),
			array('type' => 'th', 'data' => "Name", 'class' => 'hidden-sm'),
			array('type' => 'th', 'data' => "Author", 'class' => 'visible-lg'),
			array('type' => 'th', 'data' => "Version", 'class' => 'visible-lg'),
	...
	if (empty($sub_dir_names)) {
		$table->addNewRow(array(array('data' => "There are no themes in the system.", "colspan" => "5")), "themesTableTableDefaultRow", "greenRow");
	} else {
		...
		foreach($sub_dir_names as $name => $nothing) {	
			...
			if (file_exists($stylepath . '/' . $name . '/themedetails.php')) { 
				...
				$table->addNewRow(array(
					array('data' => "<img src=\"" . ((is_file($stylepath . '/' . $name . '/preview.png')) ? SITE_URL . '/' . $stylepath . '/' . $name . "/preview.png" : "images/nopreview.png") . "\" alt=\"Preview\" />", 'class' => 'center'),
					array('data' => $themeName, 'class' => 'hidden-sm'),
					array('data' => $themeAuthor, 'class' => 'visible-lg'),
					array('data' => $themeVersion, 'class' => 'visible-lg'),
					array('data' => (($currenttheme == $name) ? '<a class="btn btn-primary" onClick="return false;">Current Theme</a>' : '<a  class="btn btn-default" href="' . il( $menuvar['THEMES'] . '&action=change_theme&theme=' . $name ) . '">Activate Theme</a>'), 'class' => 'center')
				), "", "row" . $x);
