 ...
 function user_access($string, $account = NULL, $reset = FALSE) {
	...
	$userLevel = (isset($_SESSION['userid'])) ? $_SESSION['user_level'] : ANONYMOUS_ACCESS;
	 ...
	 if (!isset($perm[$userID])) {
		...
		$result = $ftsdb->select(DBTABLEPREFIX . "permissions", "name = :name", array(
			":name" => $string
		), 'name');
		...
		if ($result) {
			...
			// Add it to the DB
			$result = $ftsdb->insert(DBTABLEPREFIX . 'permissions', array(
				"name" => $string
			));

			if ($userLevel != SYSTEM_ADMIN) return false;
		}
		...
		$result = $ftsdb->select(DBTABLEPREFIX . "permissions", "role_ids LIKE :role_ids", array(
			":role_ids" => '%' . $userLevel . ',%'
		), 'name');
...
function delete_permision_setting($name) {
	...
	$result = $ftsdb->delete(DBTABLEPREFIX . 'permissions', "name = :name", array(
		":name" => $name
	));
}
...
function permision_setting_exists($name) {
	...
	$exists = 0;
	$results = $ftsdb->select(DBTABLEPREFIX . "permissions", "name = :name", array(
		":name" => $name
	));
...
function printEditUserRolePermissionsForm() {
	...
	$table->addNewRow(array(array('data' => "User Role Permissions", "colspan" => "20")), '', 'title1', 'thead');
	...
	// Add our data
	if (!$result) {
		$table->addNewRow(array(array('data' => "There are no permissions in the system.", "colspan" => "20")), "editUserRolePermissionsTableDefaultRow", "greenRow");
	} else {
		foreach ($result as $row) {
			$rowDataArray = array(
					array('data' => $row['name'])
			...
			foreach ($roles as $id => $name) {
				array_push($rowDataArray, array('data' => "<input type=\"checkbox\" name=\"" . $row['id'] . "['" . $id . "']\" value=\"1\"" . testChecked(true, ((strpos($row['role_ids'] . ',', $id . ',') === false) ? false : true) ) . " />"));
			}
