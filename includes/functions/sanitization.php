...
function keeparraysafe( $array ) {
	...
	if ( is_array( $array ) ) {
		foreach ( $array as $key => $value ) {
			if ( is_array( $value ) ) {
				$cleanArray[ keeptasafe( $key ) ] = keeparraysafe( $value );
			} else {
				$cleanArray[ keeptasafe( $key ) ] = keeptasafe( $value );
			}
		}
	}
...
function fts_check_invalid_utf8( $string, $strip = false ) {
	...
	static $is_utf8;
	if ( ! isset( $is_utf8 ) ) {
		$is_utf8 = in_array( get_config_value( 'ftsmbp_charset' ), [ 'utf8', 'utf-8', 'UTF8', 'UTF-8' ] );
	}
	if ( ! $is_utf8 ) {
		return $string;
	}
