...
class PclZip {
	...
	function create( $p_filelist ) {
		...
		if ( $v_size > 1 ) {
			...
			if ( ( is_integer( $v_arg_list[0] ) ) && ( $v_arg_list[0] > 77000 ) ) {
				...
				// ----- Parse the options
				$v_result = $this->privParseOptions( $v_arg_list,
					$v_size,
					$v_options,
					array(
	...
	function extract() {
		...
		// ----- Call the extracting fct
		$p_list   = [];
		$v_result = $this->privExtractByRule( $p_list,
			$v_path,
			$v_remove_path,
			$v_remove_all_path,
			$v_options );
	...
	function extractByIndex( $p_index ) {
		...
		$v_options_trick = [];
		$v_result        = $this->privParseOptions( $v_arg_trick,
			sizeof( $v_arg_trick ),
			$v_options_trick,
	...
	function privAddFileList( $p_filedescr_list, &$p_result_list, &$p_options ) {
		...
		for ( $j = 0; ( $j < sizeof( $p_filedescr_list ) ) && ( $v_result == 1 ); $j ++ ) {
			...
			if ( ( $p_filedescr_list[ $j ]['type'] == 'file' )
				...
				// ----- Add the file
				$v_result = $this->privAddFile( $p_filedescr_list[ $j ],
					$v_header,
					$p_options );
				if ( $v_result != 1 ) {
					return $v_result;
    ...
    */
	...
	function privWriteFileHeader( &$p_header ) {
		...
		// ----- Packed data
		$v_binary_data = pack( "VvvvvvVVVvv",
			0x04034b50,
	...
	function privWriteCentralFileHeader( &$p_header ) {
		...
		// ----- Packed data
		$v_binary_data = pack( "VvvvvvvVVVvvvvvVV",
			0x02014b50,
			$p_header['version'],
			$p_header['version_extracted'],
			$p_header['flag'],
