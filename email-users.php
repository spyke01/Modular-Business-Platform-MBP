...
if ( user_access('email_users_access') ) {
	...
	if ( $actual_action == "editEmailTemplate" && isset( $actual_id ) && user_access('email_templates_edit') ) {
		...
		if (user_access('email_templates_access')) $JQueryReadyScripts .= returnEmailTemplatesTableJQuery();
		if (user_access('email_templates_create')) $JQueryReadyScripts .= returnNewEmailTemplateFormJQuery(1);
	}
