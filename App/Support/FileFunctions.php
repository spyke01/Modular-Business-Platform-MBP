...
class FileFunctions {
	...
	function downloadFile( $file, $to ) {
		...
		if ( ! $fp ) {
			return [ 'status' => 'error', 'message' => sprintf( 'Could not open handle for fopen() to %s', $to ) ];
		}
	...
	//=================================================
	function unzip_file_ziparchive( $file, $to, $needed_dirs = [] ) {
		$z = new ZipArchive();

		// PHP4-compat - php4 classes can't contain constants
		$zopen = $z->open( $file, /* ZIPARCHIVE::CHECKCONS */ 4 );
		if ( true !== $zopen ) {
			return [ 'status' => 'error', 'message' => 'Incompatible Archive.' ];
		}
		...
		for ( $i = 0; $i < $z->numFiles; $i ++ ) {
			if ( ! $info = $z->statIndex( $i ) ) {
				return [ 'status' => 'error', 'message' => 'Could not retrieve file from archive.' ];
			}
		...
		if ( defined( 'IN_CRON' ) && IN_CRON ) {
			...
			if ( $available_space && ( $uncompressed_size * 2.1 ) > $available_space ) {
				return [ 'status' => 'error', 'message' => 'Could not copy files. You may have run out of disk space.' ];
			}
		}
		...
		foreach ( $needed_dirs as $_dir ) {
			...
			{
				return [ 'status' => 'error', 'message' => "Could not create directory. $_dir" ];
			}
		}
		unset( $needed_dirs );
		...
		for ( $i = 0; $i < $z->numFiles; $i ++ ) {
			...
			if ( false === $contents ) {
				return [ 'status' => 'error', 'message' => 'Could not extract file (' . $info['name'] . ') from archive.' ];
			}

			if ( ! $this->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE ) ) {
				return [ 'status' => 'error', 'message' => 'Could not copy file (' . $to . $info['filename'] . ').' ];
			}
		}
	...
	//=================================================
	function unzip_file_pclzip( $file, $to, $needed_dirs = [] ) {
		...
		if ( ! is_array( $archive_files ) ) {
			return [ 'status' => 'error', 'message' => 'Incompatible Archive.' . $archive->errorInfo( true ) ];
		}

		if ( 0 == count( $archive_files ) ) {
			return [ 'status' => 'error', 'message' => 'Empty archive.' ];
		}
		...
		foreach ( $archive_files as $file ) {
			...
			if ( '_notes/' === substr( $file['filename'], 0, 7 ) ) // Skip the dreamweaver created _notes directory
			{
				continue;
			}
		...
		foreach ( $archive_files as $file ) {
			...
			if ( ! $this->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE ) ) {
				return [ 'status' => 'error', 'message' => 'Could not copy file (' . $to . $file['filename'] . ').' ];
			}
		}
		...
		foreach ( (array) $dirlist as $filename => $fileinfo ) {
			if ( 'f' == $fileinfo['type'] ) {
				if ( ! $this->copy( $from . $filename, $to . $filename, true ) ) {
					...
					if ( ! $this->copy( $from . $filename, $to . $filename, true ) ) {
						return [ 'status' => 'error', 'message' => "Could not copy file ($to $filename)." ];
					}
				}
			} elseif ( 'd' == $fileinfo['type'] ) {
				if ( ! is_dir( $to . $filename ) ) {
					if ( ! mkdir( $to . $filename, FS_CHMOD_DIR ) ) {
						return [ 'status' => 'error', 'message' => "Could not create directory ($to $filename)." ];
					}
				}
