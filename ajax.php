...
if ($actual_action == "updateitem" && user_access($actual_action)) {
	...
	if ($_SESSION['user_level'] == SYSTEM_ADMIN || ($_SESSION['user_level'] == APPLICATION_ADMIN && ($table != "config" || $table != "products" || $table != "users"))) {
		...
		//==================================================
		callModuleHook('', 'handleAJAX', array(
			'section' => 'before'
		));
		
		$result = $ftsdb->update($table, array(
				$item => $updateto
			), 
			"id = :id", array(
				":id" => $actual_id
...
elseif ($actual_action == "getitem" && user_access($actual_action)) {		
	...
	if ($_SESSION['user_level'] == SYSTEM_ADMIN || ($_SESSION['user_level'] == APPLICATION_ADMIN && ($table != "config" || $table != "products" || $table != "users"))) {
		...
		$result = $ftsdb->select($table, "id = :id", array(
			":id" => $actual_id
		), $item);
		if ($result) {
...
elseif ($actual_action == "deleteitem" && user_access($actual_action)) {
	...
	if ($_SESSION['user_level'] == SYSTEM_ADMIN || ($_SESSION['user_level'] == APPLICATION_ADMIN && ($table != "config" || $table != "products" || $table != "users"))) {
		...
		if ($table == "menus") {
			$result = $ftsdb->delete(DBTABLEPREFIX . "menu_items", "menu_id = :id", array(
				":id" => $actual_id
			));
		...
		$result = $ftsdb->delete($table, "id = :id", array(
			":id" => $actual_id
		));
		$errorCount += ($result) ? 0 : 1;
...
elseif ($actual_action == "updateCategory" && user_access($actual_action)) {
	...
	$result = $ftsdb->update(DBTABLEPREFIX . "categories", array(
			"name" => $name,
			'type' => $type,
			"color" => $color,
			"tags" => $tags,
...
elseif ($actual_action == "editEmailTemplate" && user_access('email_templates_edit')) {
	...
	$result = $ftsdb->update(DBTABLEPREFIX . "email_templates", array(
			"name" => $name,
			"subject" => $subject,
			"message" => $message,
		), 
...
elseif ($actual_action == "updateMenuItem" && user_access($actual_action)) {
	...
	if (count($menuitempermissions)) {
		$role_ids = str_replace( array( "\'", "'" ), '', implode( ',', array_keys( $menuitempermissions ) ) ) . ',';
	}
...
elseif ($actual_action == "createUser" && user_access('users_create')) {
	if ( !canHaveMultipleUsers() && getUserCount() > 0 ) {
		...
		if ($password == $password2) {		
			$result = $ftsdb->insert(USERSDBTABLEPREFIX . 'users', array(
...
elseif ($actual_action == "editUser" && user_access('users_edit')) {
	...
	if ( canAccessUser($actual_id) ) {
		...
		if ($password == $password2) {			
			...
			$result = $ftsdb->update(USERSDBTABLEPREFIX . "users", $userData, 
				"id = :id", array(
					":id" => $actual_id
				)
			);
...
elseif ($actual_action == "changePassword" && user_access($actual_action)) {
	...
	if ( empty($password) || empty($password2) ) {		
		...
		$result = $ftsdb->update(USERSDBTABLEPREFIX . "users", array(
				"password" => md5($password)
			), 
			"id = :id", array(
				":id" => $_SESSION['userid']
		...
		if ( $password == $password2 ) {			
			...
			if ( $result ) {
				if ( $mbp_config['ftsmbp_require_account_activation'] ) {
					...
					if ( !$emailResult ) { echo 'Failed to send activation email!!'; }
...
elseif ( $actual_action == "forgotPassword" ) {
	...
	$result = $ftsdb->select( USERSDBTABLEPREFIX . "users", 'email_address = :email_address OR username = :username', array(
		':email_address' => $email_address,
		':username' => $username,
	) );
	if ($result && count($result) == 1) {
		...
		if ( $row['token_date'] >= mysqldatetime( strtotime( '-5 minutes' ) ) ) {
			...
			if ( !$emailResult ) { $content .= return_error_alert( 'Failed to send password reset email!!' ); }	
		}			
	} else {
...
elseif ($actual_action == "editUserRolePermissions" && user_access($actual_action)) {
	...
	foreach ($_POST as $field => $value) {
		if (is_numeric($field) && count($value)) {
			$permissions[$field] = str_replace(array("\'", "'"), '', implode(',', array_keys($value))) . ',';
		}
	}
	
	// Remove all permissions
	$result = $ftsdb->update(DBTABLEPREFIX . "permissions", array(
		"role_ids" => ''
	), '1' );
	...
	if (user_access('runUpdates')) {	
		...
		if ( count( $updateResult ) ) {
			foreach ( $updateResult as $prefix => $resultArray ) {
				if ( $resultArray['status'] == 'error' ) $returnVar .= "$prefix - ";
		...
		if ( count( $updateResult ) ) {
			foreach ( $updateResult as $theme => $resultArray ) {
				if ($resultArray['status'] == 'error' ) $returnVar .= "$theme - ";
				$returnVar .= $resultArray['message'] . "<br />";
			}
		}
		$returnVar .= "<br />";
...
elseif ($actual_action == "save-widget" && user_access($actual_action)) {
	...
	if (isset($_POST['delete_widget'])) {
		// Add to the DB
		$result = $ftsdb->delete(DBTABLEPREFIX . "widgets", "widget_id = :widgetID", array(
			":widgetID" => $widget_id
		));
		//echo "$sql<br />";
	} elseif ($_POST['create'] == 'yes') {
		// Add to the DB
		$result = $ftsdb->insert(DBTABLEPREFIX . 'widgets', array(
			"widget_id" => $widget_id,
			"area" => $area,
			'type' => $type,
			"settings" => $settings
...
elseif ($actual_action == "test" && user_access($actual_action)) {
	...
	$result = $ftsdb->select(DBTABLEPREFIX . 'clients', "first_name = :search", array(
		":search" => "%a%"
	));
	//var_export($result);
	if ($result) {
	...
	// UPDATE
	//---------------
	$result = $ftsdb->update(DBTABLEPREFIX . 'clients', array(
			"Age" => 24
		), 
		"FName = :fname AND LName = :lname", array(
			":fname" => "Jane",
			":lname" => "Doe"
		)
	);
	...
	// DELETE
	//---------------
	$result = $ftsdb->delete(DBTABLEPREFIX . 'clients', "LName = :lname", array(
		":fname" => "Jane",
		":lname" => "Doe"
	));
	if ($result) { echo "Num Rows: $result"; }
	...
	// INSERT
	//---------------
	$result = $ftsdb->insert(DBTABLEPREFIX . 'clients', array(
		"FName" => "John",
		"LName" => "Doe",
		"Age" => 26,
		"Gender" => "male"
