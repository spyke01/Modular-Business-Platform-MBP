let graphs_fillColor = 'rgba(220,220,220,0.5)';
let graphs_strokeColor = 'rgba(220,220,220,1)';
...
$(document).ready(function() {
  ...
  */
  ...
  if ($('ul#graphLinks').length) {
    let ctx = $('#graphsChart').get(0).getContext('2d');
  ...
  if ($('div.selectionPreview').length) {
    $('div.selectionPreview').each(function() {
      ...
      let id = this.id;
      let selectBox = id.replace('Preview', '');
...
function ajaxInstallModule(prefix) {
  let holderDivID = 'statusButtonHolder_' + prefix;
...
function addEditable(fields, options) {
  $.each(fields, function() {
    // Determine our settings
    let id = this;
    let table = id.replace(/edit-([a-zA-Z_]+?)-([0-9]+?)_(.*)/, '$1');
    let dbID = id.replace(/edit-([a-zA-Z_]+?)-([0-9]+?)_(.*)/, '$2');
    let name = id.replace(/edit-([a-zA-Z_]+?)-([0-9]+?)_(.*)/, '$3');
