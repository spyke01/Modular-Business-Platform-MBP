...
if ( isset( $_GET['s'] ) ) {
	$actual_section = keepsafe( $_GET['s'] );
}
if ( isset( $_GET['id'] ) ) {
...
if ( isset( $actual_style ) && $actual_style == "printerFriendly" ) {
	$page->setTemplateVar( 'Template', 'printerFriendlyTemplate.php' );
} // Page Specific Template
elseif ( file_exists( "themes/" . $mbp_config['ftsmbp_theme'] . "/template-" . $actual_page_id . ".php" ) ) {
	$page->setTemplateVar( 'Template', 'template-' . $actual_page_id . '.php' );
} // Base Template
else {
	$page->setTemplateVar( 'Template', 'template.php' );
}
...
if ( isset( $_SESSION['userid'] ) ) {
	...
	if ( ! empty( $actual_section ) ) {
		$message .= ', Section: ' . $actual_section;
	}
	if ( ! empty( $actual_prefix ) ) {
		$message .= ', Prefix: ' . $actual_prefix;
	}
	if ( ! empty( $actual_module_page ) ) {
		$message .= ', Module Page: ' . $actual_module_page;
	}
	if ( ! empty( $actual_page ) ) {
		$message .= ', Page: ' . $actual_page;
	}
	addLogEvent( [
		'type'     => LOG_TYPE_PAGE,
		'message'  => $message,
...
if ( A_VALID_LICENSE || $actual_page_id == "version" ) {
	...
	if ( $_SESSION['user_level'] != SYSTEM_ADMIN && $_SESSION['user_level'] != APPLICATION_ADMIN && $mbp_config['ftsmbp_active'] != ACTIVE ) {
		...
		if ( $actual_page_id == "activateAccount" ) {
			...
			include BASEPATH . '/activate-account.php';
			...
			if ( ! $_SESSION['username'] ) {
				...
				if ( user_access( 'email_users_access' ) && $actual_section === "emailUsers" ) {
					...
					include BASEPATH . '/email-users.php';
					...
					if ( $actual_section === "" || ! isset( $actual_section ) ) {
						...
						include BASEPATH . '/notifications.php';
					}
				} else {
			...
			include BASEPATH . '/create-account.php';
			...
			include BASEPATH . '/forgot-password.php';
	...
	$page->setTemplateVar( 'PageContent', A_VALID_LICENSE_TEXT );
}
